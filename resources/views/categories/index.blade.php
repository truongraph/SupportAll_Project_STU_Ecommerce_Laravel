                                ...
                                <div class="row">
                                    <div class="col-lg-12 col-md-12">
                                        <ul class="pagination-box">
                                            @if($currentPage > 1)
                                            <li><a href="{{ route('categories.index') }}?sort={{ request()->query('sort') }}&page=1">Trang đầu</a></li>
                                            <li><a href="{{ route('categories.index') }}?sort={{ request()->query('sort') }}&page={{ $currentPage - 1 }}">Trước</a></li>
                                            @endif

                                            @for($i = 1; $i <= $totalPages; $i++)
                                            <li class="{{ $currentPage == $i ? 'active' : '' }}"><a href="{{ route('categories.index') }}?sort={{ request()->query('sort') }}&page={{ $i }}">{{ $i }}</a></li>
                                            @endfor

                                            @if($currentPage < $totalPages)
                                            <li><a href="{{ route('categories.index') }}?sort={{ request()->query('sort') }}&page={{ $currentPage + 1 }}">Sau</a></li>
                                            <li><a href="{{ route('categories.index') }}?sort={{ request()->query('sort') }}&page={{ $totalPages }}">Trang cuối</a></li>
                                            @endif
                                        </ul>
                                    </div>
                                </div>
